...
O workspace usa Monaco, com código **somente leitura**, diff lado a lado, abas e explorador recolhível que abre por hover/foco e pode ser fixado. O cabeçalho compacto prioriza a área de código. Cmd/Ctrl+P abre arquivos; Cmd/Ctrl+click ou F12 navega para definições JS/TS, inclusive fora do diff; Alt+F12 mostra um preview. Python, Java, C#, Go, Rust, C, C++ e PHP também têm previews de declarações candidatas por análise sintática, sem exigir servidores externos. Passe o mouse para destacar a linha inteira. Clique ou arraste diretamente sobre o código, números ou + para selecionar um intervalo (Alt + arraste seleciona texto; Cmd/Ctrl + clique continua navegando): o formulário abre logo abaixo, dentro do diff. O editor tem abas **Escrever/Prévia**, barra de ferramentas Markdown (incluindo tabelas, tarefas e sugestões), atalhos Cmd/Ctrl+B/I/K e desfazer/refazer. Imagens entram por URL; anexos locais ainda não são enviados. Os rascunhos persistem no SQLite; confira e edite em **Comentários (N)** e publique uma única revisão com **Aplicar tudo**. Nada é enviado ao salvar o rascunho. O comentário salvo permanece visível abaixo das linhas, com Markdown e botão para editar, inclusive ao trocar de arquivo ou reabrir a PR.

Na lista de arquivos, **Cmd/Ctrl + clique** adiciona/remove itens da seleção e **Shift + clique** seleciona um intervalo de arquivos visíveis. O clique direito aplica ações à seleção inteira; mudar filtros ou a árvore limpa a seleção. O modo foco desativa a seleção múltipla.

Use **Concordo**, **Discordo** ou **Não li** no cabeçalho do arquivo ou no menu de clique direito. Nas pastas, a decisão se aplica aos arquivos alterados descendentes, inclusive ocultos por filtros. Concordar/discordar exige diff disponível; **Não li** também permite registrar arquivos não suportados. Essas decisões são locais, sem aprovar ou solicitar alterações no GitHub. O menu também permite marcar como visto ou pendente. Arquivos também aceitam **Aprovado sem ler**, separado da leitura e guardado no SQLite para o resumo futuro. As pastas mostram automaticamente o progresso de todos os arquivos alterados abaixo delas; aprovações sem leitura têm indicador próprio.

**Configurações → Ativar modo foco** habilita uma revisão sequencial, começando no primeiro arquivo pendente. A decisão é salva antes de avançar automaticamente. Árvore, abas, busca e atalhos não permitem pular ou voltar a outros arquivos; ações em lote ficam desativadas. Prévias de definições continuam disponíveis. **Sair do modo foco** libera a navegação a qualquer momento. A preferência persiste no SQLite e começa desativada. “Não li” registra uma decisão sem contar como leitura nem aprovação.

...
Comentários locais salvos são exibidos em cartões sob suas linhas no diff, nos lados base/head, com Markdown e edição inline. A indicação de rascunho pendente diferencia esses cartões de comentários publicados. A persistência continua no SQLite; troca de arquivo e reabertura da PR restauram os cartões.

As duas árvores de arquivos aceitam Cmd/Ctrl+clique para seleção descontínua e Shift+clique para intervalos na ordem visual, excluindo pastas recolhidas. Cmd/Ctrl+Shift acrescenta um intervalo. Seleção múltipla não troca o arquivo aberto; o clique normal volta à seleção única e navega. O clique direito preserva um grupo selecionado ou seleciona somente o arquivo clicado quando ele estava fora do grupo. Ações em lote afetam apenas arquivos alterados da PR. Filtros e troca de árvore/base-head limpam a seleção; o modo foco impede a seleção múltipla.
